...
Saved configurations are written to `mudae_kakera_configs.json` next to the script. In the `.exe` build, they are written next to `Mudae Badge Setter.exe`. That file is ignored by git so your local presets stay local.

Clicking a saved configuration loads it into the UI. Saving again with the same name updates it.

The app also remembers the last visible user ID, delay, badge counts, and selected configuration name in `mudae_kakera_last_state.json` when it closes. In the `.exe` build, this file is also written next to `Mudae Badge Setter.exe`. Those fields are restored automatically the next time you open the app.
